...
#inspect Enz_SGund table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_SGund_Hac <- filter(Enz_SGund,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_SGund$Hac%in%ac_index)))

Enz_SGund_H3K4me <- filter(Enz_SGund,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_SGund$H3K4me%in%me_index)))

Enz_SGund_H3K36me <- filter(Enz_SGund,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_SGund$H3K36me%in%me_index)))

Enz_SGund_H3K9me <- filter(Enz_SGund,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_SGund$H3K9me%in%me_index)))

Enz_SGund_H3K27me <- filter(Enz_SGund,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K27me",sum(Enz_SGund$H3K27me%in%me_index)))

Enz_SGund_H4K20me <- filter(Enz_SGund,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_SGund$H4K20me%in%me_index)))

Enz_SGund_H3K79me <- filter(Enz_SGund,H3K79me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K79me",sum(Enz_SGund$H3K79me%in%me_index)))
...
#inspect Enz_SGdiff table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_SGdiff_Hac <- filter(Enz_SGdiff,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_SGdiff$Hac%in%ac_index)))

Enz_SGdiff_H3K4me <- filter(Enz_SGdiff,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_SGdiff$H3K4me%in%me_index)))

Enz_SGdiff_H3K36me <- filter(Enz_SGdiff,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_SGdiff$H3K36me%in%me_index)))

Enz_SGdiff_H3K9me <- filter(Enz_SGdiff,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_SGdiff$H3K9me%in%me_index)))

Enz_SGdiff_H3K27me <- filter(Enz_SGdiff,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K27me",sum(Enz_SGdiff$H3K27me%in%me_index)))

Enz_SGdiff_H4K20me <- filter(Enz_SGdiff,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_SGdiff$H4K20me%in%me_index)))

Enz_SGdiff_H3K79me <- filter(Enz_SGdiff,H3K79me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme,Family)) %>% mutate(HK=rep("H3K79me",sum(Enz_SGdiff$H3K79me%in%me_index)))
...
#inspect Enz_Prel table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_Prel_Hac <- filter(Enz_Prel,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_Prel$Hac%in%ac_index)))

Enz_Prel_H3K4me <- filter(Enz_Prel,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_Prel$H3K4me%in%me_index)))

Enz_Prel_H3K36me <- filter(Enz_Prel,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_Prel$H3K36me%in%me_index)))

Enz_Prel_H3K9me <- filter(Enz_Prel,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_Prel$H3K9me%in%me_index)))

Enz_Prel_H3K27me <- filter(Enz_Prel,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K27me",sum(Enz_Prel$H3K27me%in%me_index)))

Enz_Prel_H4K20me <- filter(Enz_Prel,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_Prel$H4K20me%in%me_index)))

Enz_Prel_H3K79me <- filter(Enz_Prel,H3K79me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K79me",sum(Enz_Prel$H3K79me%in%me_index)))

Enz_Prel_H2AZK7me <- filter(Enz_Prel,H2AZK7me=="me1") %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H2AZK7me",1))
...
#inspect Enz_LZ table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_LZ_Hac <- filter(Enz_LZ,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_LZ$Hac%in%ac_index)))

Enz_LZ_H3K4me <- filter(Enz_LZ,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_LZ$H3K4me%in%me_index)))

Enz_LZ_H3K36me <- filter(Enz_LZ,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_LZ$H3K36me%in%me_index)))

Enz_LZ_H3K9me <- filter(Enz_LZ,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_LZ$H3K9me%in%me_index)))

Enz_LZ_H3K27me <- filter(Enz_LZ,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme,Family)) %>% mutate(HK=rep("H3K27me",sum(Enz_LZ$H3K27me%in%me_index)))

Enz_LZ_H4K20me <- filter(Enz_LZ,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_LZ$H4K20me%in%me_index)))

Enz_LZ_H3K79me <- filter(Enz_LZ,H3K79me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K79me",sum(Enz_LZ$H3K79me%in%me_index)))

Enz_LZ_H4K12me <- filter(Enz_LZ,H4K12me=="me1") %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K12me",1))

LZ <- rbind(Enz_LZ_Hac,Enz_LZ_H3K4me,
              Enz_LZ_H3K36me, Enz_LZ_H3K9me,
              Enz_LZ_H3K27me, Enz_LZ_H4K20me,
              Enz_LZ_H3K79me, Enz_LZ_H4K12me) 
...
#inspect Enz_PD table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_PD_Hac <- filter(Enz_PD,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_PD$Hac%in%ac_index)))

Enz_PD_H3K4me <- filter(Enz_PD,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_PD$H3K4me%in%me_index)))

Enz_PD_H3K36me <- filter(Enz_PD,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_PD$H3K36me%in%me_index)))

Enz_PD_H3K9me <- filter(Enz_PD,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_PD$H3K9me%in%me_index)))

Enz_PD_H3K27me <- filter(Enz_PD,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K27me",sum(Enz_PD$H3K27me%in%me_index)))

Enz_PD_H4K20me <- filter(Enz_PD,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_PD$H4K20me%in%me_index)))

Enz_PD_H3K79me <- filter(Enz_PD,H3K79me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K79me",sum(Enz_PD$H3K79me%in%me_index)))
...
#inspect Enz_RStid table to se how many marks correspond to the enzymes
#construct table for dotplot for each mark
Enz_RStid_Hac <- filter(Enz_RStid,Hac %in% ac_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("Hac",sum(Enz_RStid$Hac%in%ac_index)))

Enz_RStid_H3K4me <- filter(Enz_RStid,H3K4me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K4me",sum(Enz_RStid$H3K4me%in%me_index)))

Enz_RStid_H3K36me <- filter(Enz_RStid,H3K36me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K36me",sum(Enz_RStid$H3K36me%in%me_index)))

Enz_RStid_H3K9me <- filter(Enz_RStid,H3K9me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K9me",sum(Enz_RStid$H3K9me%in%me_index)))

Enz_RStid_H3K27me <- filter(Enz_RStid,H3K27me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K27me",sum(Enz_RStid$H3K27me%in%me_index)))

Enz_RStid_H4K20me <- filter(Enz_RStid,H4K20me %in% me_index) %>% 
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H4K20me",sum(Enz_RStid$H4K20me%in%me_index)))

Enz_RStid_H3K79me <- filter(Enz_RStid,H3K79me %in% me_index) %>%
  dplyr::select(c(RPKM,Symbol,Enzyme)) %>% mutate(HK=rep("H3K79me",sum(Enz_RStid$H3K79me%in%me_index)))
