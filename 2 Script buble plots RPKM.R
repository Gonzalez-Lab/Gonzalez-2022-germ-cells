...
Enz_MK <- RPKM_zsc[rownames(RPKM_zsc) %in% genelist$Symbol,] #matrix with z scores
Enz_MK_RPKM <- RPKM[rownames(RPKM) %in% genelist$Symbol,] # matrix with RPKM values
Enz_MK_RPKM <- cbind(Enz_MK_RPKM,cell_cluster=Enz_MK[,6]) #add cluster info to RPKM matrix

Enz_MK <- Enz_MK[order(Enz_MK[,"cell_cluster"]),] # order rows by cell cluster in z score matrix
Enz_MK <- as.data.frame(Enz_MK) #convert to data frame

Enz_MK_RPKM <- Enz_MK_RPKM[order(Enz_MK_RPKM[,"cell_cluster"]),] # order rows by cell cluster in RPKM matrix
Enz_MK_RPKM <- as.data.frame(Enz_MK_RPKM) #convert to data frame

head(Enz_MK)
head(Enz_MK_RPKM)

#make a heatmap with the selected data
pheatmap(Enz_MK[,1:5], fontsize = 5, cluster_cols = FALSE, cluster_rows = FALSE)

#match the order of enzymes in the two tables
genelist$order <- match(genelist$Symbol,rownames(Enz_MK))
genelist <- genelist[order(genelist$order),] # sort gene list in the Enz_MK table order

# Bind enzymes ans MK info to z score and RPKM matrices
Enz_MK <- cbind(Enz_MK, genelist[,2:12]) 
...
cluster1_H2AZK7me <- filter(cluster1,H2AZK7me=="H2AK7me") %>% 
  arrange(factor(Enzyme, levels = c("Writer","Eraser")), Family) %>%
...
cluster5_H3K79me <- filter(cluster5,H3K79me %in% me_index) %>% 
  arrange(factor(Enzyme, levels = c("Writer","Eraser")), Family) %>% 
  select(c(Sperm,Name,Enzyme,Family)) %>% mutate(HK=rep("H3K79me",1), 
                                                 Chromatin=rep("Writer",1))
